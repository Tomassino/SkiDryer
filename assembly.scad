...
module sd_assembly() {
	...
	for (p = [[-1, -1], [-1, 1], [1, 1], [1, -1]]) {
		translate([p[0] * ((sd_baseWidth - sd_legWidth) / 2 - sd_legsDistanceFromBorder), p[1] * ((sd_baseDepth - sd_legWidth) / 2 - sd_legsDistanceFromBorder), -sd_heightFromGround / 2]) {
			sd_lowerLeg();
		}
	}
...
// 		translate([sd_internalVerticalBarPosition + sd_aluminiumBarSectionSize + sd_epsilon, 0, upperSkiSupportsPosition]) {
// 			sd_upperSkiSupport();
// 		}
// 	}
//
// 	// Finally we can place the horizontal bar on top of the two vertical bars
// 	translate([(sd_externalVerticalBarPosition + sd_internalVerticalBarPosition) / 2, 0, sd_verticalBarLength + sd_planesThickness + (sd_aluminiumBarSectionSize / 2)]) {
// 		sd_horizontalBar();
// 	}
}
